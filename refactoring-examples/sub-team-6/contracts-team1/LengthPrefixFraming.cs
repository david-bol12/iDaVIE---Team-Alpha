// Sub-team 6 — LengthPrefixFraming (ADR-0002 §"Framing").
//
...
// quotes the corresponding sentence in ADR-0002.
...
{
    /// <summary>
    /// Encode / decode the length-prefixed wire frames described in ADR-0002
    ...
    public static class LengthPrefixFraming
    {
        ...
        {
            // ADR-0002: "length = byte length of the UTF-8 payload, ASCII decimal,
            // no leading zeros."
        ...
        public static async Task<byte[]> ReadOneAsync(Stream stream, CancellationToken ct = default)
        {
            ...
            while (true)
            {
                ...
                // ADR-0002: "No \r."
                if (one[0] == Cr)
                    throw new FormatException("Frame header contained CR (0x0D); ADR-0002 forbids \\r.");

                // ADR-0002: "ASCII decimal" — reject anything else early.
