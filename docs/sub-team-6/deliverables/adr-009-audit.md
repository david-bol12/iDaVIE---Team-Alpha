...
| F9 | C4 — Transport has a real consumer | `D4/README.md` §Example 1, `D4/metrics.md` §2.2, `D5/test-strategy.md` §4 (l.141–143), `D5/viewmodel-unit-tests.md` §2 | **The File-tab worked example does NOT use the transport.** It depends on four local Unity-side adapter interfaces (`IFitsService`, `IFileDialogService`, `IVolumeService`, `IMemoryProbe`). `D5/test-strategy.md` §4: *"A consolidated `IServiceGateway` façade is **not** implemented in either worked example."* This contradicts ADR-009's positive consequence: *"GUI sub-team and server sub-teams integrate via the contract"* — there is no integration. | ❌ |
| F10 | C4 — Debug stream over transport | `D4/README.md` §Example 2, `D4/metrics.md` §3.2 | Debug tab uses an in-process `UnityLogStreamAdapter` subscribing to `Application.logMessageReceived`, **not** server-pushed `log.emit` notifications. `D1/requirements.md` §2 (l.23) promises Debug exercises the "server-pushed stream"; the actual implementation is local. | ❌ |
...
| F12 | C5 — Headless test execution | `D4/metrics.md` §2.3 (l.162), `D5/test-strategy.md` §3.4 (l.131–137) | 63 NUnit tests (34 File + 29 Debug) run via `dotnet test`; debug-tab measured ~20 ms. Zero Unity dependency. | ✅ |
...
| F17 | D1 promise vs D4 delivery | `D1/requirements.md` §2 (l.23) vs `D4/README.md`, `D4/metrics.md` | D1 states *"File for request/response RPC, Debug for the server-pushed stream — confirming the contract has a real consumer."* Neither delivery achieves this. See F9, F10. | ❌ |
...
1. **F9 + F10 + F17 — Transport contract has no consumer.** The single biggest gap. ADR-009 names the transport (JSON-RPC over named pipes) as the integration spine between GUI and server, and D1 promises File-tab as the request/response demo and Debug-tab as the push-stream demo. D4 and D5 deliver neither. Pick one path:
   - **(a)** Reshape WE1 so `FileTabViewModel` routes file-open through `IServiceGateway.SendAsync("file.open", …)` and have `FitsServiceAdapter` live on the **server** side of the gateway in the diagrams. Mock the gateway in tier-1 tests. This is faithful to ADR-009 and to D1's promise, but requires diagram and test rework.
   - **(b)** Soften ADR-009 (and D1 line 23) to say the transport is **specified**, not exercised, in the worked examples — and explicitly defer the consumer integration to Sub-team 1 + a future sprint. Cheapest fix; truthful, but weakens the pitch.
   - **(c)** Add a small third worked example: a single `IServiceGateway` round-trip (`session.hello`) wired into a tier-1 test with a mock pipe. Cheap evidence the contract has a consumer without rewriting WE1/WE2.
