...
- - **Adapters** (Unity): `FileTabView`, `FitsServiceAdapter`, `FileDialogServiceAdapter`, `VolumeServiceAdapter`, `MemoryProbeAdapter`, `FileTabCompositionRoot`
  ...
  - - LOC: 1899 → 8 classes (~1438 total)
    - - WMC: 57 → 27 (FileTabViewModel, within domain threshold after remediation)
  ...
  - - **Decomposition:** 1 class → 8 focused classes, single responsibility each
    ...
    **Solution:** Introduce Observer pattern with ILogStream / ILogObserver interfaces:
    ...
    - - **Adapters** (Unity): `UnityLogStreamAdapter` (subscribes to static event, publishes to ILogStream), `DebugTabView` (thin view), `DebugTabCompositionRoot` (wiring)
      ...
      - **Key Metrics (BEFORE → AFTER):**
      - - LOC: 255 → 382 total (but split across 7 types)
        - - WMC: 8 → 6 (DebugTabViewModel) / 8 (unchanged in DebugLogging replaced by UnityLogStreamAdapter)
          - - CBO: ~10 → 3-7 per class (VM: 3, View: 7)
            - - LCOM4: ~3 (disjoint concerns) → 1 per class (cohesive)
                                ...
                                - - **Non-invasiveness:** ✅ All 44 existing `Debug.Log*` call sites remain unchanged; captured automatically
                                  - - **Unit testability (NFR-TST-1):** 0 → 29 NUnit tests (~20 ms total)
                                    - - **Observer pattern:** ✅ New observers (autosave, telemetry, filtering) can attach without modifying producers
                                      ...
                                      2. 2. **Domain Extraction:** Both achieve clean domain/adapter split with ACL boundaries enforced by assembly-level dependency rules.
                                         3. 3. **Zero Unit Tests → Full Coverage:** Example 1 goes 0 → 34 tests; Example 2 goes 0 → 29 tests. Both run without Unity.
                                            4. 4. **Production Unchanged:** Example 2's producer side (all `Debug.Log*` call sites) remains untouched — non-invasive refactor.
                                              
                                               5. ---
                                              
                                               6. ## Implementation Status
                                              
                                               7. - ✅ **BEFORE diagrams** (Mermaid): Both examples have class diagrams and sequence/trace documentation
                                                  - - ✅ **AFTER diagrams** (Mermaid): Both examples have class diagrams and sequence diagrams
                                                    - - ✅ **Dependency graphs**: Text-based DSM analysis with Section 4.2 compliance verification
                                                      - - ✅ **CK metrics**: Hand-counted (pending Day 13 tool verification)
                                                        - - ✅ **Skeleton code**: FileTabViewModel, DebugTabViewModel, adapters, composition roots
                                                          - - ✅ **Unit tests**: 34 file-tab tests, 29 debug-tab tests
                                                            - - ⏳ **Tool verification**: Planned for Day 13 (NDepend, Understand, DV8, CodeScene)
